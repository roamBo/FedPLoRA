# Branch B �?FedPLoRA-SVD（FedSVD 思路：服务端正交 A_0 + 聚合�?SVD 重分解）

## B.0 一句话定位

> **把初�?`A_0` �?QR 正交化为 row-orthonormal 矩阵 (rows �?d 维单位向量、互相正�?，聚合后�?stacked-A �?SVD 重分解，让所有客户端�?A_down 都对齐到同一组主成分坐标�?*。预期：B 的更新方向不�?A 的非正交�?扭曲"，跨客户端的 A 聚合更稳定�?
## B.1 假设与预�?
| 假设 | 检验方�?|
|---|---|
| H-B.1 仅做 A_0 初始正交化就能让 A 聚合更稳�?| B1 vs v2 |
| H-B.2 聚合�?SVD 重分解能进一步提�?| B2 vs B1 |
| H-B.3 SVD 后的 A 与本地训�?B 仍兼容（核心风险�?| B2 看是否退�?|

预期收益�?
- B1 A_0 正交�?*+0.000 ~ +0.003**（小提升�?- B2 SVD 重分解：**+0.002 ~ +0.006**（中等提升，但有崩溃风险�?
## B.2 设计要点

### B.2.1 A_0 初始正交�?
PEFT 默认 LoRA A �?Kaiming uniform 初始化，**不是正交**。改动：
...
### B.2.3 B 兼容性问�?
SVD 后的�?`A_new` 行方向可能与客户端的 `B_i` 训练时假设的 A 行方向不一�?—�?`B_i` 在原 A 列上学到了某�?row 系数，SVD 后行被重组，`B_i` 的列对应关系被打乱�?
**两种缓解**�?
1. **Procrustes 对齐**（FedRot-LoRA 思路）：�?`R = argmin ||R · A_new �?A_mean||_F` s.t. `R^T R = I`，然后下�?`R · A_new`。这样新 A 既保�?SVD 的正交性又最大程度对齐客户端历史方向�?
2. **客户�?first-batch 快速校�?*：下发后客户端在 forward 时先固定 A_new �?1 �?batch �?B-only gradient step，再开始评估。但这破坏了 "no extra round" 叙事，仅�?fallback�?
## B.3 超参建议
...
- 初始化改造：[utilities/v4_orth_init.py](../../../utilities/v4/v4_orth_init.py)
- 服务端聚合：[methods/fedplora_v4_svd.py](../../../methods/v4/fedplora_v4_svd.py)
...
1. **正交 A 影响 LoRA scaling**：PEFT 内部 `lora_alpha / lora_r` 假设 A �?norm �?*某个* 经验值；强制正交化后�?norm 可能偏离。需�?scale 后再写回�?2. **SVD 重分解破�?B-A 兼容**：B �?v2 的关键私有量，A 全部换主成分坐标�?B 可能"失效"。Procrustes 是缓解手段，但有上限�?3. **数值稳�?*：N·r �?d �?SVD 退化；Llama-3.1-8B q_proj �?d=4096，N·r = 35×8 = 280 << 4096，安全。但�?r=16 或客户端数增加，需要换 randomized SVD�?