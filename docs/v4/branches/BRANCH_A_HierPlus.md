# Branch A �?FedPLoRA-Hier++（修�?v3 退化的硬修复版�?
## A.0 一句话定位

> �?v3 的残�?+ 簇路径基础上，**去掉 quantile 硬阈、改 soft sigmoid blending、加 spectral 数据驱动簇，并修复评估时 cluster A_down 没正确加载的实现缺陷**，目标是让残差分支真正服务于跨域个性化而不�?丢弃 20% row"�?
## A.1 假设与预�?
| 假设 | 检验方�?|
|---|---|
| H-A.1 *soft gate* �?*hard quantile gate* 表现更好（不丢信息） | A1 vs v3-cluster |
| H-A.2 *spectral cluster* �?*先验 3 �? 表现更好（数据驱动） | A2 vs A1 |
| H-A.3 评估时严格按 client_id 加载 personalized A 能恢�?v3 损失的个性化粒度 | A3 vs v3-cluster |
| H-A.4 Hier++ �?Macro �?�?v2，在 Worst �?�?v2 | A1/A2/A3 vs v2 |

预期收益（基�?v2 base 0.69595 macro_acc）：

- A1 soft prior cluster�?*+0.000 ~ +0.003**（修 v3，但�?cluster 仍为 3 簇，提升有限�?- A2 spectral K=5�?*+0.002 ~ +0.005**（数据驱动簇适配 7 域）
- A3 �?eval�?*�?A1 持平或略�?*（验证机制本身）

## A.2 �?v3 的差�?
| 维度 | v3 (`fedplora_oneshot.py:_aggregate_v3`) | v4 Branch A |
|---|---|---|
| gate 形式 | `gate = where(conflict > threshold, sigmoid((c−�?/T), 0)` | `gate = sigmoid((c �?μ_c) / (σ_c · κ))`，无硬阈 |
| 阈值来�?| layer-wise quantile q=0.80 | 标准化后的连�?sigmoid，温�?κ �?[0.5, 2] 可调 |
| 簇划�?| 固定 prior 3 �?| `prior` / `spectral_k` / `kmeans_k` / `none` 多�?|
| 簇个性化范围 | residual 簇均 | residual 簇均 + 行内 row-importance 加权 |
| 评估�?personalized A | 加载 ✓，�?default 路径用簇�?| 加载 ✓，�?default 路径�?*unweighted-cluster* 全平均，避免和簇路径同质 |
...
- **`gate` 始终连续**，conflict 越高 gate 越大，但不强�?0 / 1�?- **`spectral_cluster`** 直接�?`[vec(R_i)]` 矩阵�?(N×D) �?SVD top-K �?K-Means，K∈{3,5,7}�?- **`default_A`** 不再使用簇均，避免与个性化路径同质化（这是 v3 �?bug）�?- **per-client A_down** 严格�?`client_id` 写入 `personalized_states`，evaluation 时正确加载�?
## A.4 超参建议
...
- 服务端聚合：[methods/fedplora_v4_hier.py](../../../methods/v4/fedplora_v4_hier.py)
- 客户�?local 训练：与 v2 fedplora-oneshot 完全一致（复用 `_add_fedplora_oneshot_anchor`�?- 训练入口：[tasks/fed_train_sft_v4.py](../tasks/fed_train_sft_v4.py) �?`agg_type �?{v4_hier_soft_prior, v4_hier_soft_spectral, v4_hier_soft_pfl_eval}`
- 运行脚本：[scripts/RunScripts/run_v4_branch_a.sh](../../../scripts/RunScripts/run_v4_branch_a.sh)

## A.6 风险

1. spectral cluster �?N=35 客户端、D=O(10M) 的客户端残差上要内存�?35·40MB �?1.4 GB�?*先在一层做 SVD 降维**�?100 维再聚类�?2. soft gate 可能�?gate 始终 �?.5（中间值），如果是这种情况下，�?`--v4_gate_power=2` �?gate �?两极�?�?3. spectral cluster K 选错会损害效果，A2 �?K∈{3,5,7} 三档先做 pilot 选最优�?