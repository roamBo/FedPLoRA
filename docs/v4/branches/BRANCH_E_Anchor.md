...
> **服务端持有一个小型公开 anchor 数据集（每域 50 条公开 instruction），聚合后用它在 *candidate λ / threshold / rank* 网格上选最优配�?*。引入一个轻�?假设"（服务端有少量公共数据）�?+0.005 ~ +0.01 macro_acc 的潜在上限。是否纳入主论文取决于审稿人对该假设的接受度�?
## E.1 假设与预�?
| 假设 | 检验方�?|
|---|---|
| H-E.1 50 �?�?�?anchor 足以稳定选出最�?λ | E1 vs grid search 实验 |
| H-E.2 anchor-driven gate threshold > 固定 threshold | E1 vs A1 |
| H-E.3 anchor-driven cluster lambda > 固定 lambda | E2 vs A1 |
| H-E.4 �?anchor 不破�?worst-domain 表现 | �?branch 对照 |

预期收益（基�?Branch A best）：
...
放在 `data/anchor_seed/seed_42/anchor.jsonl`�?50 条总样本�?
## E.3 候选配置网�?
```python
candidates = []
...
= 81 个候选。每个候选评�?50 batches × 7 �?�?350 forwards。在 Llama-3.1-8B 上单 forward ~ 80 ms，共 350 × 81 × 0.08 �?38 min。可接受�?
## E.4 算法
...
## E.5 �?LoraHub 的差�?
LoraHub �?black-box optimization �?*任务模块* 之间搜组合权重；E 是在 *超参* 之间搜组合，且只�?350 个公共样本�?
## E.6 风险

1. anchor 数据质量：若 anchor 不代表客户端真实分布，选出�?config �?anchor-optimal 但不�?client-optimal。需要做 sanity check：anchor loss �?client val loss 的相关�?�?0.5�?2. anchor data 引入审稿争议。建议在论文里给出两套结果：with-anchor �?without-anchor，让审稿人选�?3. 如果 anchor �?size > 50/域，会触�?"anchor 等于 mini-dataset" 的非 one-shot 担忧�?*严格 cap �?50/�?*�?
## E.7 实现入口

- Anchor data loader：[utilities/anchor_data.py](../../../utilities/v4/anchor_data.py)
- 服务端候选搜索：[methods/fedplora_v4_anchor.py](../../../methods/v4/fedplora_v4_anchor.py)
