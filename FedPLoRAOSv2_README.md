...
`fed_train_sft.py` 在 11.1 中给出 **12** 条可整段复制的命令（对应 **12** 个 `agg_type`）。其中 **`fedplora-oneshot`** 与 **`yoco`** 都是单轮方法，但已经**分叉**：`fedplora-oneshot` 现在走 **FedPLoRA-Oneshot** 的冲突门控聚合（以初始共享 `A0` 为参考，结合 row-importance 与冲突回退），`yoco` 仍是 `aggregate_models_yoco` 的 PCWA 方案。别名：`fedsa`≡`fedsa_lora`，`fd_lora`≡`fdlora`，`het_lora`≡`hetlora`，`loraa2`≡`lora_a2`。
...
#### 5) `fedplora-oneshot`（FedPLoRA-Oneshot 冲突门控；入口会强制 `--rounds 1`）
...
**聚合类型说明**：多轮 **`fedplora`** 为 **FedP-LoRA**（客户端按 FedP 协议上传 LoRA `A` 与可训练头、行统计；服务端 `aggregate_models_fedplora`；本地带对齐/近端/正交正则）。`fedplora-oneshot` 为 **FedPLoRA-Oneshot**：同样只上传 `A` 与头、`B` 留在本地，但服务端不再做 YOCO-PCWA，而是对每层 `A` 进行**基于初始 `A0` 的行级冲突门控**，结合样本量、row-importance 与一致性权重聚合；高冲突行会回退到 `A0`，默认不做 QR 旋转，以保持 one-shot 下 `B` 的行坐标兼容。该方法和 `yoco` 已经是两条不同路径。
...
已有 `result/*.log` 可以直接补充一批论文可用指标，不需要重新训练或重新评测：

```bash
cd /Users/hawaiii/codex/FedPLoRa/FedPLoRA-v2
python scripts/analyze_existing_results.py \
  --result_dir result \
  --output_dir result \
  --main_method fedplora-oneshot
```

输出文件：

- `result/analysis_summary.md`：主表、相对提升、per-domain loss、指标充分性说明。
- `result/analysis_summary.csv`：每个 run 的最终/最优/通信/鲁棒性指标。
- `result/analysis_rounds.csv`：逐轮指标，适合画收敛曲线和多轮退化曲线。
- `result/analysis_summary.json`：同 summary 表，便于后续脚本读取。

离线脚本会从日志中派生以下指标：

- **宏平均性能**：`best_domain_macro_loss`、`final_domain_macro_loss`。
- **鲁棒性**：`best_worst_domain_loss`、`final_domain_loss_std`、`final_domain_loss_gap`、`final_domain_loss_cv`。
- **高风险域平均**：`finance/legal/medical` 的 `final_hard_domain_macro_loss`。
- **能力域平均**：`code/math` 的 `final_capability_domain_macro_loss`。
- **通信量**：由 `[setup] comm_down_bytes_per_client`、`comm_up_bytes_per_client`、客户端数、实际轮数计算 `final_total_comm_gb`。
- **效率与退化**：对多轮方法给出 `macro_degradation_final_minus_best` 和 `worst_degradation_final_minus_best`，避免最后一轮比最佳轮差却被忽略。

当前已有日志的限制也要如实写清：

- 已有旧日志中 `fedplora-oneshot` 是旧版 **FedP+YOCO-PCWA** 结果，不对应现在新增的 conflict-gated FedPLoRA-Oneshot。
- 旧日志只稳定包含 loss 与通信量；如果没有 token accuracy、个性化本域/跨域指标，脚本会留空，不能凭空补。
- 顶会主表可以先用这些离线指标讲趋势，但最终版最好至少补 3 个 seed 或小规模 7c/14c seed sweep。

### 16.2 现有指标是否够用

**短期够用**：若目标是完成方法论验证和论文初稿，现有 `domain_macro_loss + worst_domain_loss + per-domain loss + communication` 已能支撑“跨域冲突下的平均收益、最坏域鲁棒性、通信效率”三条主线。

**最终版不够**：顶会/顶刊审稿通常会追问稳定性和真实生成质量，因此建议最终补充：

- **多 seed 均值±方差**：至少 `seed=42/43/44`，可以只在 7c 或 14c 做成本可控版本。
- **个性化指标**：新实验加 `--eval_personalization_metrics`，报告 `client_local_macro_loss`、`in_domain_domain_test_macro_loss`、`off_domain_macro_loss`。
- **token-level accuracy / PPL**：新代码已经支持，但旧日志可能没有；新跑时自动写入 JSON。
- **消融**：`--oneshot_no_keep_init_on_conflict`、`--oneshot_orthogonalize`、不同 `--oneshot_conflict_threshold`。
- **生成式指标**：如果时间允许，在每域抽样做 exact match / Rouge-L / pass@k，这部分需要额外解码脚本，不能从现有 loss 日志恢复。
...
## 20260510：FedPLoRA-Oneshot v2 落地与结果复用

### 目标故事

FedPLoRA-Oneshot v2 的核心定位是：**只通信一次，在不上传私有 `B_i` 的情况下，用共享 `A` 承载跨域公共知识，用本地 `B_i` 保留个性化知识，并通过服务端行级冲突门控避免不同领域的 `A_i` 直接平均互相抵消**。

这条线和已有工作的区分点：

- 相比 **FedSA-LoRA**：不是简单平均 `A`，而是利用 `A0` 参考方向、row-importance 和冲突分数做行级选择性聚合。
- 相比 **YOCO**：不上传/聚合 `B`，也不做 PCWA；重点是 `B_i` 本地私有和 `A` 的跨域冲突门控。
- 相比多轮 **FedPLoRA**：只有一次上传和一次下发，适合通信受限或隐私更强的跨域个性化场景。

### 已落地代码

- 新增 `methods/fedplora_oneshot.py`：`aggregate_models_fedplora_oneshot`。
- 修改 `tasks/fed_train_sft.py`：`fedplora-oneshot` 不再走 `aggregate_models_yoco`，而是走独立冲突门控聚合器。
- 修改 `utilities/train_eval.py`：新增 one-shot 的 `A0` 行方向锚定正则 `_add_fedplora_oneshot_anchor`。
- 修改 `utilities/utils.py`：通信统计将 `fedplora-oneshot` 的 row-importance 统计计入上行。
- 新增 `scripts/analyze_existing_results.py`：复用已有日志生成更详细评价指标。

### 方法细节

对每个 LoRA `A` 矩阵的每一行：

1. 以初始化后广播前的 `A0` 为参考，先对客户端上传的 `A_i` 做符号对齐，避免 LoRA 因子符号不确定性导致平均抵消。
2. 用样本量、`B_i` 推导的 row-importance、与 `A0` 的方向一致性共同决定客户端行权重。
3. 计算行冲突分数：`conflict = 1 - ||weighted_mean_direction||`。
4. 当冲突超过 `--oneshot_conflict_threshold` 时，将该行按门控强度回退到 `A0`，避免跨域互斥知识污染全局 `A`。
5. 默认不做 QR 正交化，因为 one-shot 下旋转 `A` 的行空间会破坏已经在本地训练好的 `B_i` 行坐标兼容性。
...
入口会自动把 `--rounds 10` 强制成 1，因此这里保留 10 只是为了和其它脚本参数一致。日志中会打印：

```text
[fedplora-oneshot] conflict mean=... max=... high_row_frac=... init_gate=...
```

这些字段也会写入 metrics JSON 的 `fedplora_oneshot_conflict`。
...
- `result/analysis_summary.md`
- `result/analysis_summary.csv`
- `result/analysis_rounds.csv`
- `result/analysis_summary.json`

注意：已有旧日志中的 `fedplora-oneshot` 是旧版 **FedP+YOCO-PCWA** 结果，只能作为旧实现分析；新增 v2 conflict-gated 实现的真实性能需要后续至少做一个小规模验证。离线脚本的价值是最大化复用已有昂贵实验，不是把旧结果改造成新方法结果。

### 建议补充的低成本验证

如果时间很紧，不建议马上重跑 35c 全量。优先顺序：

1. **7c smoke**：确认新聚合器能完整训练、评估、写出 conflict stats。
2. **7c 对比**：`fedplora-oneshot` vs `yoco` vs `fedsa_lora`，同 seed。
3. **35c 单次主实验**：只跑新 `fedplora-oneshot`，用于替换旧版 FedP+YOCO 的主方法结果。
4. **消融**：只在 7c 做 `--oneshot_no_keep_init_on_conflict` 和 `--oneshot_orthogonalize`。
...
## 附录：FedP-LoRA 多轮 vs FedPLoRA-Oneshot（`fedplora-oneshot`）

### 本仓库中的实现对应关系

| 项目 | `fedplora`（多轮 FedP-LoRA） | `fedplora-oneshot`（FedPLoRA-Oneshot） |
|------|------------------------------------------|----------------------------------------|
...
| 服务端聚合 | `methods/fedp_lora.py`：`aggregate_models_fedplora`（符号对齐、动量、QR 等） | `methods/fedplora_oneshot.py`：`aggregate_models_fedplora_oneshot`（初始 `A0` 符号对齐、row-importance、行级冲突门控、高冲突回退到 `A0`） |
| 本地训练额外项 | `utilities/train_eval.py`：FedP 对齐 / 近端 / 正交（依赖广播的全局 `A`） | `A` 稀疏项（`--yoco_sparse_lambda`，作为 one-shot 稀疏先验复用）+ `A0` 行方向锚定（`--oneshot_anchor_lambda`） |
| 独立 `yoco` 模式 | — | `agg_type=yoco` 仍走 `methods/yoco.py` 的 PCWA；它是 one-shot baseline，不再与 `fedplora-oneshot` 共享聚合器 |

入口逻辑见 `tasks/fed_train_sft.py`（`is_fedplora_multiround_agg` vs `is_fedplora_oneshot_agg` / `is_yoco_agg`）、`utilities/utils.py`（`is_fedplora_oneshot_agg`）、`utilities/train_eval.py`（`_add_yoco_sparse` 和 `_add_fedplora_oneshot_anchor` 对 `fedplora-oneshot` 生效）。
