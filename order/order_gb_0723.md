# FedPLoRA 投稿主张校准与缺口实验命令（2026-07-23，gb 服务器）

> 由 `order/order_20260723.md` 适配。章节与命令格式保持原文；仅换服务器路径、conda、GPU，并写入 gb 已知坑。

######### FedPLoRA 1-shot 全折复核、3B 协议对齐与 spectrum 状态生成（gb 单卡 GPU1 版）-20260723 #########
...
GPU: 全部物理 1 号卡（原 GPU_ID=0/1 均改为 1）；同卡多条 nohup 会抢 GPU1，资源紧时注释未跑行或等前一条结束
...
4. 单卡串行：G1–G12 / R1–R3 / C2–C7 同卡一次只开一条；3B 独占 GPU1。
5. personalized-eval 不加 --force_retrain；G14–G16 必须保留 --force_retrain。
6. HF 不可达：export HF_ENDPOINT=https://hf-mirror.com。
7. spectrum / common-test 在 gb 上仍用 dir05 states；勿把 minghao A100 9k checkpoint 混进来。
8. 新脚本用 git pull 同步，勿依赖 Mac scp 到 minghao 路径。
```
...
P1: D1 dir05 / 35 clients / 7 domains / v13a one-round states / CPU layer spectrum（gb 用 domain_benchmark，勿与 minghao A100 9k 混报）
```
...
确保含：`l1_layer_spectrum.py`、`pt_reader.py`、`build_common_test_benchmark.py`、
`checkpoint_manifest.py`、`run_external_lm_eval.py` 及更新后的 `eval_personalized.py`、`fed_train_sft.py`。
...
export GPU_ID=${GPU_ID:-1}  # gb 单卡：固定 1
...
cd /data/yaominghao/gb/FedPLoRA && export PATH="/data/yaominghao/miniconda3/envs/fedplora/bin:${PATH}" && python /data/yaominghao/gb/FedPLoRA/scripts/Analysis/checkpoint_manifest.py --roots $CKPT_SEARCH_ROOTS --output "$CKPT_MANIFEST"

for SEED in 42 43 44; do
  export "CKPT_NORMAL_${SEED}=$(python /data/yaominghao/gb/FedPLoRA/scripts/Analysis/checkpoint_manifest.py --roots $CKPT_SEARCH_ROOTS --resolve --agg_type normal --seed "$SEED" --model_contains SmolLM2-135M --benchmark_contains "/data/domain_benchmark_35c_dir05/seed_${SEED}")"
  export "CKPT_V13A_${SEED}=$(python /data/yaominghao/gb/FedPLoRA/scripts/Analysis/checkpoint_manifest.py --roots $CKPT_SEARCH_ROOTS --resolve --agg_type fedplora_v13a_os --seed "$SEED" --model_contains SmolLM2-135M --benchmark_contains "/data/domain_benchmark_35c_dir05/seed_${SEED}")"
done
```
...
for SEED in 42 43 44; do
  python /data/yaominghao/gb/FedPLoRA/scripts/Analysis/build_common_test_benchmark.py \
...
这里要求 19k dir0.5 与 19k IID；gb 不要用 minghao 的 A100 9k checkpoint；只用本机 dir05 训练产物。builder 会做 `source train/val ∩ common test` 的 source-id/prompt 级泄漏检查并在非零交集时硬停止；此时必须从冻结 shared test 重新构建全部划分并重训，不能强行 eval-only。hybrid manifest 中的 train/client 文件哈希必须来自 dir0.5，test 文件哈希必须来自 IID。
...
Stage 4（P1，主张锁定后再跑）
  gb 单卡：G14–G16 串行，一 seed 结束再开下一 seed
