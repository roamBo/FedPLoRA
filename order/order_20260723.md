# FedPLoRA 20260723 投稿前统一实验命令（合并修正版）

######### FedPLoRA 1-shot、router、common-test修复、YOCO与补充实验统一入口-20260723 #########

> 这是 `order_20260723.md` 与 `order_20260723_sup.md` 的合并修正版。以后 0723 批次只按本文档运行，不再单独打开 sup 文档执行。  
> 合作者刚才卡住的 common-test 报错是有效的数据泄漏保护，不是脚本小错误；不要绕过。正确修复路径是第七部分 `7.2-A0 -> A1-A6`：冻结 IID full test，重分配 dir0.5 的非测试池，然后重训 Normal/v13a。  
> 需要额外新跑的 YOCO、70-client、r16 实验统一放在最后的第九部分之后；先完成前面的 P0 主线和修复，再按资源追加。
...
原主线 GPU 任务共 **16 条**：12 + 1 + 3。第七部分另补 3 条 offset0 router、正文方案 6 条 full-common-test dir0.5 重训练和 2 条 external-task eval。旧 checkpoint 的 intersection eval-only 路径已停用，避免再次撞到 train/test overlap 报错。额外新增实验统一放到本文最后。
...
scp -o RemoteCommand=none -o RequestTTY=no \
  /Users/hawaiii/codex/FedPLoRA/FedPLoRA-main/scripts/DataProcessScripts/repartition_with_frozen_test.py \
  minghao@172.26.191.30:/data2/minghao/code/FedPLoRA-main/scripts/DataProcessScripts/
```
...
```bash
MODEL_PATH="$MODEL_135M" BENCHMARK_DIR_MAIN="$DIR05_COMMON_ROOT/seed_42" EXPECTED_NUM_CLIENTS=35 RUN_TAG_DATASET=dir05_common_test_v2 PIPELINE_ROUNDS=1 PIPELINE_EVAL_MAX_BATCHES=0 nohup /usr/bin/time -v bash scripts/RunScripts/run_20260713_one_experiment.sh --kind sft --method N9_common_dir05_normal_seed42 --agg normal --seed 42 --split-seed 42 --run-id-prefix common_test_20260723_dir05 --gpu "${GPU_ID:-0}" -- --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_common_dir05_normal_seed42.log" 2>&1 &
```

```bash
MODEL_PATH="$MODEL_135M" BENCHMARK_DIR_MAIN="$DIR05_COMMON_ROOT/seed_42" EXPECTED_NUM_CLIENTS=35 RUN_TAG_DATASET=dir05_common_test_v2 PIPELINE_ROUNDS=1 PIPELINE_EVAL_MAX_BATCHES=0 nohup /usr/bin/time -v bash scripts/RunScripts/run_20260713_one_experiment.sh --kind sft --method N7_ours_common_dir05_v13a_seed42 --agg fedplora_v13a_os --seed 42 --split-seed 42 --run-id-prefix common_test_20260723_dir05 --gpu "${GPU_ID:-0}" -- --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_common_dir05_v13a_seed42.log" 2>&1 &
```

```bash
MODEL_PATH="$MODEL_135M" BENCHMARK_DIR_MAIN="$DIR05_COMMON_ROOT/seed_42" EXPECTED_NUM_CLIENTS=35 RUN_TAG_DATASET=dir05_common_test_v2 PIPELINE_ROUNDS=1 PIPELINE_EVAL_MAX_BATCHES=0 nohup /usr/bin/time -v bash scripts/RunScripts/run_20260713_one_experiment.sh --kind sft --method N9_common_dir05_normal_seed43 --agg normal --seed 43 --split-seed 43 --run-id-prefix common_test_20260723_dir05 --gpu "${GPU_ID:-0}" -- --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_common_dir05_normal_seed43.log" 2>&1 &
```

```bash
MODEL_PATH="$MODEL_135M" BENCHMARK_DIR_MAIN="$DIR05_COMMON_ROOT/seed_42" EXPECTED_NUM_CLIENTS=35 RUN_TAG_DATASET=dir05_common_test_v2 PIPELINE_ROUNDS=1 PIPELINE_EVAL_MAX_BATCHES=0 nohup /usr/bin/time -v bash scripts/RunScripts/run_20260713_one_experiment.sh --kind sft --method N7_ours_common_dir05_v13a_seed43 --agg fedplora_v13a_os --seed 43 --split-seed 43 --run-id-prefix common_test_20260723_dir05 --gpu "${GPU_ID:-0}" -- --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_common_dir05_v13a_seed43.log" 2>&1 &
```

```bash
MODEL_PATH="$MODEL_135M" BENCHMARK_DIR_MAIN="$DIR05_COMMON_ROOT/seed_42" EXPECTED_NUM_CLIENTS=35 RUN_TAG_DATASET=dir05_common_test_v2 PIPELINE_ROUNDS=1 PIPELINE_EVAL_MAX_BATCHES=0 nohup /usr/bin/time -v bash scripts/RunScripts/run_20260713_one_experiment.sh --kind sft --method N9_common_dir05_normal_seed44 --agg normal --seed 44 --split-seed 44 --run-id-prefix common_test_20260723_dir05 --gpu "${GPU_ID:-0}" -- --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_common_dir05_normal_seed44.log" 2>&1 &
```

```bash
MODEL_PATH="$MODEL_135M" BENCHMARK_DIR_MAIN="$DIR05_COMMON_ROOT/seed_42" EXPECTED_NUM_CLIENTS=35 RUN_TAG_DATASET=dir05_common_test_v2 PIPELINE_ROUNDS=1 PIPELINE_EVAL_MAX_BATCHES=0 nohup /usr/bin/time -v bash scripts/RunScripts/run_20260713_one_experiment.sh --kind sft --method N7_ours_common_dir05_v13a_seed44 --agg fedplora_v13a_os --seed 44 --split-seed 44 --run-id-prefix common_test_20260723_dir05 --gpu "${GPU_ID:-0}" -- --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_common_dir05_v13a_seed44.log" 2>&1 &
```
...
export CKPT_NORMAL_EXT_42="$(python scripts/Analysis/checkpoint_manifest.py --roots $CKPT_SEARCH_ROOTS --resolve --agg_type normal --seed 42 --model_contains SmolLM2-135M --benchmark_contains "A100_domain_benchmark_35c_dir05/seed_42")"
export CKPT_V13A_EXT_42="$(python scripts/Analysis/checkpoint_manifest.py --roots $CKPT_SEARCH_ROOTS --resolve --agg_type fedplora_v13a_os --seed 42 --model_contains SmolLM2-135M --benchmark_contains "A100_domain_benchmark_35c_dir05/seed_42")"
...
结果根:
/data2/minghao/result/FedPLoRA/order_0723_sup

模型根:
/data2/minghao/model/trained_models_LW/order_0723_sup

模型:
/data2/minghao/model/SmolLM2-135M
/data2/minghao/model/SmolLM2-1.7B
/data2/minghao/model/Qwen2.5-3B
...
/data2/minghao/code/FedPLoRA-main/data/A100_domain_benchmark_70c_dir05_frozen_test/seed_{42,43,44}
...
```text
run_logs:
/data2/minghao/result/FedPLoRA/order_0723_sup/launcher_logs/test20260723_sup_*.log

result_logs:
/data2/minghao/result/FedPLoRA/order_0723_sup/<run_id>/result_logs/<method>/

result_files/client_states:
/data2/minghao/result/FedPLoRA/order_0723_sup/<run_id>/result_files/client_states/<method>/

checkpoints:
/data2/minghao/model/trained_models_LW/order_0723_sup/<run_id>/<method>/

pids:
/data2/minghao/result/FedPLoRA/order_0723_sup/pids/*.pid
```
...
export CODE_DIR=/data2/minghao/code/FedPLoRA-main
export RESULT_ROOT=/data2/minghao/result/FedPLoRA/order_0723_sup
export MODEL_ROOT=/data2/minghao/model/trained_models_LW/order_0723_sup
export MODEL_135M=/data2/minghao/model/SmolLM2-135M
export MODEL_17B=/data2/minghao/model/SmolLM2-1.7B
export MODEL_3B=/data2/minghao/model/Qwen2.5-3B
export D1_ROOT=/data2/minghao/code/FedPLoRA-main/data/A100_domain_benchmark_35c_dir05
export FLOWER_ROOT=/data2/minghao/code/FedPLoRA-main/data/domain_benchmark_flowertune_mixed_20c_dir05
export D1_70C_ROOT=/data2/minghao/code/FedPLoRA-main/data/A100_domain_benchmark_70c_dir05_frozen_test
export GPU_ID=${GPU_ID:-0}
...
```bash
CUDA_VISIBLE_DEVICES='' python scripts/RunScripts/print_sft_comm_profile.py \
  --model "$MODEL_135M" \
  --lora_r 16 \
  --lora_alpha 32 \
...
```bash
python scripts/Analysis/summarize_fedplora_results.py \
...
```bash
CUDA_VISIBLE_DEVICES="${GPU_ID:-0}" nohup /usr/bin/time -v python -u tasks/fed_train_sft.py --model "$MODEL_135M" --benchmark_dir "$D1_ROOT/seed_42" --num_clients 35 --agg_type fedalt "${COMMON_SFT_ARGS[@]}" "${LORA_R16_ARGS[@]}" --client_state_dir "$RESULT_ROOT/r16_fedalt_seed42/result_files/client_states/N2_r16_fedalt_seed42" --metrics_output_dir "$RESULT_ROOT/r16_fedalt_seed42/result_logs/N2_r16_fedalt_seed42" --save_run_checkpoint_dir "$MODEL_ROOT/r16_fedalt_seed42/N2_r16_fedalt_seed42" --trained_models_root "$MODEL_ROOT/r16_fedalt_seed42" --eval_max_batches 0 --seed 42 --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_sup_r16_fedalt_seed42.log" 2>&1 &
...
```bash
CUDA_VISIBLE_DEVICES="${GPU_ID:-0}" nohup /usr/bin/time -v python -u tasks/fed_train_sft.py --model "$MODEL_135M" --benchmark_dir "$D1_ROOT/seed_43" --num_clients 35 --agg_type normal "${COMMON_SFT_ARGS[@]}" "${LORA_R16_ARGS[@]}" --client_state_dir "$RESULT_ROOT/r16_normal_seed43/result_files/client_states/N2_r16_normal_seed43" --metrics_output_dir "$RESULT_ROOT/r16_normal_seed43/result_logs/N2_r16_normal_seed43" --save_run_checkpoint_dir "$MODEL_ROOT/r16_normal_seed43/N2_r16_normal_seed43" --trained_models_root "$MODEL_ROOT/r16_normal_seed43" --eval_max_batches 0 --seed 43 --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_sup_r16_normal_seed43.log" 2>&1 &
...
```bash
CUDA_VISIBLE_DEVICES="${GPU_ID:-0}" nohup /usr/bin/time -v python -u tasks/fed_train_sft.py --model "$MODEL_135M" --benchmark_dir "$D1_ROOT/seed_44" --num_clients 35 --agg_type normal "${COMMON_SFT_ARGS[@]}" "${LORA_R16_ARGS[@]}" --client_state_dir "$RESULT_ROOT/r16_normal_seed44/result_files/client_states/N2_r16_normal_seed44" --metrics_output_dir "$RESULT_ROOT/r16_normal_seed44/result_logs/N2_r16_normal_seed44" --save_run_checkpoint_dir "$MODEL_ROOT/r16_normal_seed44/N2_r16_normal_seed44" --trained_models_root "$MODEL_ROOT/r16_normal_seed44" --eval_max_batches 0 --seed 44 --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_sup_r16_normal_seed44.log" 2>&1 &
...
```bash
CUDA_VISIBLE_DEVICES="${GPU_ID:-0}" nohup /usr/bin/time -v python -u tasks/fed_train_sft.py --model "$MODEL_135M" --benchmark_dir "$D1_ROOT/seed_44" --num_clients 35 --agg_type fedalt "${COMMON_SFT_ARGS[@]}" "${LORA_R16_ARGS[@]}" --client_state_dir "$RESULT_ROOT/r16_fedalt_seed44/result_files/client_states/N2_r16_fedalt_seed44" --metrics_output_dir "$RESULT_ROOT/r16_fedalt_seed44/result_logs/N2_r16_fedalt_seed44" --save_run_checkpoint_dir "$MODEL_ROOT/r16_fedalt_seed44/N2_r16_fedalt_seed44" --trained_models_root "$MODEL_ROOT/r16_fedalt_seed44" --eval_max_batches 0 --seed 44 --force_retrain > "$RESULT_ROOT/launcher_logs/test20260723_sup_r16_fedalt_seed44.log" 2>&1 &
...
root = pathlib.Path("/data2/minghao/result/FedPLoRA/order_0723_sup")
expected = [
...
Stage 3（P0/P1，可与 eval-only 跨机并行）
  ...
  在 172.26.191.30 按 order_20260723 跑 H1 common-test 与 R1 router/onboarding 验收。
