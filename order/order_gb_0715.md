# FedPLoRA 20260715 大规模补表命令（gb 服务器）

> 由 `order/order_Baseline_20260715.md` 适配。超参数、算法与实验设计不变；仅修改路径、conda、**全部 GPU=1**（gb 单卡串行，不做 minghao 侧 0–3 多卡并行）。逐条粘贴后台 nohup，无 `wait`。

######### FedPLoRA-OS 大规模 baseline / public data / scale 实验命令（gb 单卡 GPU1 版）-20260715 #########
...
默认环境: conda fedplora
默认主模型: /data/yaominghao/gb/models/SmolLM2-135M
规模确认模型: /data/yaominghao/gb/models/SmolLM2-1.7B
...
GPU: 全部物理 1 号卡（CUDA_VISIBLE_DEVICES=1）；gb 单卡，所有 GPU 实验串行
```
...
G1 smoke:     v13_20260715_scale_1p7b_smoke_seed42
G1 baseline:  scale_20260715_baseline_1p7b_d1_split42_core8_r1_finaleval_seed42
G1 ours:      scale_20260715_ours_1p7b_d1_split42_core8_r1_finaleval_seed42
G2:           flowertune_20260715_core8_seed{42,43,44}
G6:           nonIID_20260715_{baseline,ours}_{iid,dir01}_r1_finaleval_seed42

run_logs:
/data/yaominghao/gb/result/FedPLoRA/<RUN_ID>/run_logs/

result_logs:
/data/yaominghao/gb/result/FedPLoRA/<RUN_ID>/result_logs/

result_files:
/data/yaominghao/gb/result/FedPLoRA/<RUN_ID>/result_files/

trained checkpoints:
/data/yaominghao/gb/models/trained_models_LW/<RUN_ID>/
```

【preflight 防坑（0711/0712 已踩过）】

```text
1. 禁止同壳连续 source baseline + main：后一次会盖掉角色白名单与默认前缀。
2. 正确顺序：先 source 对应 preflight → 再 export RUN_ID_PREFIX / SMOKE_RUN_ID
   → 再 set_run_paths <seed> 或 refresh_smoke_paths → 再跑实验。
3. source 时若已 export 过 RUN_ID_PREFIX/SMOKE_RUN_ID，common 会保留（${VAR:-default}）；
   但 source 后仍建议再 export 一次并 set_run_paths，避免看错目录。
4. 每条 run_sft_* 会 echo RUN_ID / SMOKE_RUN_ID / log= 路径；必须含 20260715，禁止 20260709。
5. 单卡：export GPU=1 后行首不要再写 GPU=0；同时只跑一个 GPU 实验。
```
...
```text
scripts/RunScripts/preflight_20260709_common.sh   # 需含 RUN_ID 保留 + refresh_smoke_paths
scripts/RunScripts/preflight_20260709_baseline.sh
scripts/RunScripts/preflight_20260709_main_algorithm.sh
scripts/RunScripts/run_20260713_one_experiment.sh
...
## 0.2 路径与环境（每次开新 shell 先执行；不要双 source）

**只 export 路径与 GPU，不要在这里同时 source baseline 和 main。** 各实验段再 source 对应角色。
...
以下命令均假设已执行 §0.2。`export GPU=1` 后，行首**不要**再写 `GPU=0`；可写 `GPU=1` 或省略（用已 export 的值）。

## 1. G1 smoke：SmolLM2-1.7B CORE-8

说明：先只跑 smoke，确认大模型路径、显存、LoRA target_modules、checkpoint 写入都正常。MMLU 生成式指标当前仓库没有独立脚本，本命令只产出 FedPLoRA 框架内 token-acc / communication / local metrics；MMLU 需后续用 lm-eval 或单独脚本补评。**1.7B 显存紧，必须串行，逐条粘贴。**
...
run_sft_smoke smoke_fedsa_lora fedsa_lora
run_sft_smoke smoke_fedalt fedalt
run_sft_smoke smoke_hydralora hydralora
run_sft_smoke smoke_fedlease fedlease
```

验收：

```bash
ls -la "$RESULT_ROOT/v13_20260715_scale_1p7b_smoke_seed42/run_logs/"
find "$RESULT_ROOT/v13_20260715_scale_1p7b_smoke_seed42/result_logs" -name '*.json' | sort
# 若出现 baseline_20260709_smoke_seed42，说明 SMOKE_RUN_ID 未在 source 后重设
```
...
mkdir -p \
  "$RESULT_ROOT/v13_20260715_scale_1p7b_smoke_seed42/run_logs" \
  "$RESULT_ROOT/v13_20260715_scale_1p7b_smoke_seed42/result_logs"

# 等上一条结束再贴下一条
SMOKE_RUN_ID_20260713=v13_20260715_scale_1p7b_smoke_seed42 \
nohup bash scripts/RunScripts/run_20260713_one_experiment.sh \
  --kind smoke --method smoke_v13a_os_1p7b --agg fedplora_v13a_os --gpu 1 -- --force_retrain \
  > "$RESULT_ROOT/v13_20260715_scale_1p7b_smoke_seed42/run_logs/launch_v13a.log" 2>&1 &
echo "[ours-smoke] v13a pid=$!"

SMOKE_RUN_ID_20260713=v13_20260715_scale_1p7b_smoke_seed42 \
nohup bash scripts/RunScripts/run_20260713_one_experiment.sh \
  --kind smoke --method smoke_v13b_os_bonly_1p7b --agg fedplora_v13b_os_bonly --gpu 1 -- --force_retrain \
  > "$RESULT_ROOT/v13_20260715_scale_1p7b_smoke_seed42/run_logs/launch_v13b.log" 2>&1 &
echo "[ours-smoke] v13b pid=$!"
```
...
source scripts/RunScripts/preflight_20260709_baseline.sh
# 必须在 source 之后设置，否则会落到 baseline_20260709_35c_dir05_r10_finaleval_seed42
...
# 必须看到 scale_20260715_baseline_1p7b_..._seed42

run_sft_full N7_baseline_normal_1p7b normal
run_sft_full N7_baseline_ecolora_1p7b ecolora --ecolora_keep_ratio 0.25 --ecolora_mask_mode round_robin
run_sft_full N7_baseline_fedsa_lora_1p7b fedsa_lora
run_sft_full N7_baseline_fedalt_1p7b fedalt
run_sft_full N7_baseline_hydralora_1p7b hydralora
...
# 必须在 source 之后设置，否则会落到 v12_20260709_main_..._seed42
export RUN_ID_PREFIX=scale_20260715_ours_1p7b_d1_split42_core8_r1_finaleval
set_run_paths 42
echo "[check] RUN_ID=$RUN_ID RUN_ROOT=$RUN_ROOT"
# 必须看到 scale_20260715_ours_1p7b_..._seed42

run_sft_full N7_ours_v13a_os_1p7b fedplora_v13a_os
run_sft_full N7_ours_v13b_os_bonly_1p7b fedplora_v13b_os_bonly
```
...
export HF_HOME=${HF_HOME:-/data/yaominghao/gb/cache/huggingface}
export HF_DATASETS_CACHE=${HF_DATASETS_CACHE:-/data/yaominghao/gb/cache/huggingface/datasets}
...
if missing:
    raise SystemExit(
        ...
        + ",".join(missing)
        + "\\n请先在 fedplora 环境安装：python -m pip install -U datasets scikit-learn"
    )
...
```bash
tail -f /data/yaominghao/gb/result/FedPLoRA/flowertune_20260715_build/run_logs/build_flowertune_mixed_all.log
```

### 3.2 有网备选：在有网机器先构建 raw JSONL
...
```bash
rsync -avP /path/to/FedPLoRA/data/raw_flowertune_mixed.jsonl \
  yaominghao@<gb-host>:/data/yaominghao/gb/FedPLoRA/data/raw_flowertune_mixed.jsonl
```
...
说明：135M smoke，仍全部 `CUDA_VISIBLE_DEVICES=1` 串行。`CODE_DIR` 需先 export。
...
run_flower_full () {
  ...
  local log_path="$run_root/run_logs/test20260715_flower_${method}_seed${seed}.log"
  echo "[run_flower_full] method=$method run_id=$run_id log=$log_path"
  CUDA_VISIBLE_DEVICES="$gpu" nohup python -u tasks/fed_train_sft.py \
    --model "$MODEL_PATH" \
...
说明：起步只跑 Normal/FedSA/FedALT/v13a/v13b，先验证趋势。如果资源充足再补 EcoLoRA/Hydra/FedLEASE。gb 单卡串行。
...
for variant in iid dir01; do
  ...
  # source 之后再设前缀；每次循环都 set_run_paths，确保 mkdir
  export RUN_ID_PREFIX=nonIID_20260715_baseline_${variant}_r1_finaleval
  set_run_paths 42
  echo "[check] variant=$variant RUN_ID=$RUN_ID BENCHMARK_DIR=$BENCHMARK_DIR"
  run_sft_full N7_baseline_${variant}_normal normal
...
export GPU=1
source scripts/RunScripts/preflight_20260709_baseline.sh
mkdir -p /data/yaominghao/gb/result/FedPLoRA/summary_20260715

python scripts/Analysis/summarize_fedplora_results.py /data/yaominghao/gb/result/FedPLoRA \
  --exclude_smoke \
  --strict_fingerprint \
  --output /data/yaominghao/gb/result/FedPLoRA/summary_20260715/summary_all_non_smoke.md
```

# 推荐执行顺序（gb 单卡）

```text
0) git pull + §0.2 路径 export + preflight
1) §1 G1 smoke（1.7B，逐条串行）→ 确认 metrics JSON
2) §2 G1 正式 CORE-8（串行；可与 §3 数据构建错开时段）
3) §3 FlowerTune 构建（0 GPU，可与 smoke 并行）
4) §4 FlowerTune smoke → §5 正式（先 seed42）
5) §6 非 IID 构建 → §7 5 方法 × 2 变体
6) §8 summarize
```

【注意事项】

1. G1 大模型先跑 smoke；正式命令只有在 smoke 写出 metrics JSON 后再启动。1.7B 务必串行，勿并行抢 GPU1。
